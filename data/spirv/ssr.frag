...
const int   MAX_RAY_MARCH_MAX_SAMPLES            = 32;

layout (constant_id = 1)
const float RAY_MARCH_MIN_STEP_SIZE              = 0.020;

layout (constant_id = 2)
const float RAY_MARCH_MAX_STEP_SIZE              = 0.020;
...
const float FOREGROUND_TEST_BIAS                 = 0.5;

layout (constant_id = 4)
const int   RAY_MARCH_OBSTACLE_MAX_SAMPLES       = 8;

layout (constant_id = 5)
const float RAY_MARCH_OBSTACLE_MIN_STEP_SIZE     = 0.02;
...
const float OBSTACLE_BREAK_DEPTH_DISTANCE        = 2.5;
...
const float OBSTACLE_JUMP_MIN_DISTANCE           = 4.0;
...
const float MAX_REFLECTION_DISTANCE              = 0.50;
...
{
   vec4 clip_pos   = PCB.Proj * vec4(pos, 1.0);
...
{
   z_buffer = -eye_z_from_depth(z_buffer, PCB.Proj);
   z = -eye_z_from_depth(z, PCB.Proj);
   ...
   if (z_buffer < z - FOREGROUND_TEST_BIAS) {
      ...
      z = -eye_z_from_depth(ss_out.z, PCB.Proj);
      if (z_buffer < z - FOREGROUND_TEST_BIAS)
         return true;
   }
                           ...
                           inout bool keep_going)
{
    ...
    * early.
    */
   float p_dist = -eye_z_from_depth(p_depth, PCB.Proj);
   if (p_dist < OBSTACLE_JUMP_MIN_DISTANCE) {
      out_color = FAIL_COLOR_OBSTACLE;
      keep_going = false;
      return false;
   }
   ...
   for (int i = 0; i < RAY_MARCH_OBSTACLE_MAX_SAMPLES; i++) {
      ...
      if (is_outside_screen(ss_jp.xy)) {
         out_color = FAIL_COLOR_NOT_IN_SCREEN;
         keep_going = false;
         return false;
      }
      ...
      if (ss_jp.z >= jp_depth) {
          ...
          */
         float jp_dist = -eye_z_from_depth(jp_depth, PCB.Proj);
         if (jp_dist - prev_jp_dist > OBSTACLE_BREAK_DEPTH_DISTANCE) {
            ...
            return true;
#else // case c)
            out_color = FAIL_COLOR_DOUBLE_OBSTACLE;
            keep_going = false;
            return false;
#endif
         }
...
get_step_size(int step_idx, vec3 p, float p_depth)
{
   ...
   float z_buffer = -eye_z_from_depth(p_depth, PCB.Proj);
   float z = -eye_z_from_depth(p.z, PCB.Proj);
...
get_reflection_position(vec3 ss_refl_dir, vec3 ss_p_min, out vec3 ss_p_out)
{
    ...
    * point.
    */
   out_color = FAIL_COLOR_REFLECTION_TOO_FAR;
   return false;
}
...
void main()
{
   ...
   vec4 frag_color = texture(tex_color, in_uv);
   float reflectiveness = frag_color.a;
   ...
   float eye_position_z = eye_z_from_depth(tex_depth, in_uv, PCB.Proj);
   ...
   vec3 eye_normal = texture(tex_normal, in_uv).rgb;
   ...
   vec3 ss_frag_pos = vec3(in_uv, texture(tex_depth, in_uv));
   ...
   if (refl_dist > MAX_REFLECTION_DISTANCE) {
      out_color = FAIL_COLOR_REFLECTION_DIST;
      return;
   ...
   if (dp_normal_refl_dir > MAX_DOT_REFLECTION_NORMAL) {
      out_color = FAIL_COLOR_REFLECTION_DIR;
      return;
   ...
   out_color.rgb = texture(tex_color, ss_p.xy).rgb;
   out_color.a = reflectiveness * att_cam * att_uv * att_refl_angle * att_refl_dist;
}
